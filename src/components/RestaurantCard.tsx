...
export function RestaurantCard({ restaurant }: RestaurantCardProps) {
  ...
  return (
    <div className="mb-8 flex h-90 w-80 flex-col rounded-2xl border-2 border-[var(--color-primary)] bg-[var(--color-background)] hover:cursor-pointer">
      ...
      <div className="flex w-full flex-1 flex-col items-start justify-between gap-2 px-3 py-3">
        ...
        <div className="flex w-full items-center justify-center">
          <Link to={`/view-restaurant-dishes/${restaurant.id}`}>
            <Button variant="default" className="w-70 hover:cursor-pointer">
              Ver pratos
            </Button>
          </Link>
        </div>
