...
export function RestaurantHeader() {
  ...
  return (
    <>
      <header className="flex w-full items-center justify-between bg-[var(--color-background)] sm:relative fixed z-50">
        ...
        <div className="hidden sm:flex sm:mr-8 items-center gap-7">
          <Link
          ...
          <Link
            className={`whitespace-nowrap transition hover:cursor-pointer hover:text-[var(--color-primary)] ${location.pathname === "/restaurants" ? "border-b-4 border-[var(--color-primary)] text-[var(--color-primary)] " : ""}`}
            to="/create-dish"
          >
            CADASTRAR PRATO
          </Link>
        ...
        <div className="fixed top-20 right-0 min-h-screen bg-[var(--color-background)] z-40 shadow-2xl sm:hidden">
          <div className="flex flex-col gap-4 px-6 py-4">
            ...
            <div className="flex items-center justify-between pt-2">
              <Link to="/edit-restaurant-profile" onClick={() => setIsOpen(false)}>
